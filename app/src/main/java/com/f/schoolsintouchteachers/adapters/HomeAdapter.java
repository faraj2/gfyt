...
import com.f.schoolsintouchteachers.models.Uploadaholders;
...
public class HomeAdapter extends RealmRecyclerViewAdapter<Pst, RecyclerView.ViewHolder> {
    Context context;
    ...
    private SparseArray<Uploadaholders> tsks = new SparseArray<>();
    ...
    private UploadTask taskk = null;
    UploadTask task = null;
    Activity activity;


    public HomeAdapter(Realm realm, Context context, @Nullable OrderedRealmCollection<Pst> data, boolean autoUpdate, FragmentManager childFragmentManager,Activity activity) {
    ...
    private void compressingVid(Uri uri, final String pic, int position) {
...
//                ffmeg.execFFmpegBinary(command);
        ...
        VideoCompress.VideoCompressTask videoCompressTask = VideoCompress.compressVideoLow(input, f, new VideoCompress.CompressListener() {
            public byte[] d;
...
//                            d = byteArrayOutputStream.toByteArray();
                            ...
                            task.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                                ...
                                public void onComplete(@NonNull Task<Uri> task) {
                                    if (task.isSuccessful()) {
                                        ...
                                        taskk = storageReference.putFile(viduri);
        ...
        tsks.put(position, new Uploadaholders(videoCompressTask, taskk, position, task,null));
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            case 0:
                view1 = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_wall, parent, false);
                return new Myviewholder(view1);
            case 1:
                view1 = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_audio, parent, false);
                return new AudioViewholder(view1);
        }
    ...
    public int getItemViewType(int position) {
        switch (getData().get(position).getPost1().getType()) {
            case 0:
                return 0;
            case 1:
                return 1;
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder holder, final int position) {
        ...
        if (pst != null && pst.isValid()) {
            switch (pst.getPost1().getType()) {
                case 0:
                    ...
                    if (pst.isPosting()) {
                        if (postin != null) {
                            if (!postin.contains(position)) {
                                ...
                                myviewholder.donut2.setVisibility(View.VISIBLE);
                                myviewholder.donut2.setOnClickListener(new View.OnClickListener() {
                                    @Override
                                        ...
                                        builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                                            ...
                                            public void onClick(DialogInterface dialog, int which) {
                                                ...
                                                if (tsks != null && tsks.size() > 0) {
                                                    if (tsks.get(position).getVideoCompressTask() != null && tsks.get(position).getVideoCompressTask().getStatus() == AsyncTask.Status.RUNNING) {
                                                        tsks.get(position).getVideoCompressTask().cancel(true);

                                                    } else if (tsks.get(position).getVideoCompressTask() != null && tsks.get(position).getVideoCompressTask().getStatus() == AsyncTask.Status.PENDING) {
                                                        tsks.get(position).getVideoCompressTask().cancel(true);

                                                    } else if (tsks.get(position).getTask() != null && tsks.get(position).getTask().isInProgress()) {
                                                        tsks.get(position).getTask().cancel();

                                                    } else if (tsks.get(position).getTaskpic() != null && tsks.get(position).getTaskpic().isInProgress()) {
                                                        tsks.get(position).getTaskpic().cancel();

                                                    }
                                                    realm.executeTransaction(new Realm.Transaction() {
                                                        @Override
                                                        public void execute(Realm realm) {
                                                            Pst ps = realm.where(Pst.class).equalTo("id", getData().get(position).getId()).findFirst();
                                                            ps.deleteFromRealm();
                                ...
                                handler = new Handler() {
                                    ...
                                    public void handleMessage(Message msg) {
                                        ...
                                        myviewholder.donut2.setProgress(p);

                                        if (msg.obj != null) {
                                            String pass = (String) msg.obj;
                                            if (pass.equals("image")) {
                                                myviewholder.donut2.setProgress(99);
                                                ...
                                                Config.firestore.document(Config.auth.getCurrentUser().getDisplayName()).collection("posts").document(id)
                                                    ...
                                                    public void onComplete(@NonNull Task<Void> task) {
                                                        if (task.isSuccessful()) {
                                                            myviewholder.donut2.setVisibility(View.GONE);
                                                ...
                                                myviewholder.donut2.setProgress(100);
                                ...
                                handler.post(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        if (pst1.getImages() != null && pst1.getImages().size() > 0) {
                                            ...
                                            if (!TextUtils.isEmpty(pic)) {
                                                myviewholder.setImage(pic);
                                                Uri uri = Uri.parse(pst1.getPost1().getVideo());
                                                compressingVid(uri, pic, position);
                                            } else {
                                                myviewholder.donut2.setProgress(0);
                    ...
                    if (pst.getImages() != null && pst.getImages().size() > 0) {
                        ...
                        myviewholder.contaoner.setVisibility(View.GONE);
                        ...
                        myviewholder.recyclerView.addItemDecoration(new CirclePagerIndicatorDecoration());
                        ...
                        myviewholder.recyclerView.setAdapter(myfadapter);
                        myfadapter.updateList(pst.getImages());

                        //   holder.recyclerView.setRecycledViewPool(viewPool);

                    } else {
                        myviewholder.contaoner.setVisibility(View.VISIBLE);
                        ...
                        if (!TextUtils.isEmpty(pst.getPost1().getVideo()) && pst.getPost1().getVideo().startsWith("https")) {
                            if (downloadholders != null && downloadholders.size() > 0) {
                                for (Downloadholders downloadholders1 : downloadholders) {
                                    if (downloadholders1.getPosition() == myviewholder.getAdapterPosition() && !downloadholders1.getFuture().isDone()) {
                                        myviewholder.ply.setVisibility(View.GONE);
                                        donutProgress3 = downloadholders1.getProgress().donutProgress;
                                        donutProgress3.setVisibility(View.VISIBLE);
                                        //  holder.donutProgress.setVisibility(View.VISIBLE);
                                    } else if (downloadholders1.getPosition() == myviewholder.getAdapterPosition() && downloadholders1.getFuture().isDone()) {
                                        downloadholders1.getProgress().donutProgress.setVisibility(View.GONE);
                                    }
                                }

                            } else {
                                myviewholder.ply.setVisibility(View.VISIBLE);
                            }

                        } else {
                            myviewholder.ply.setVisibility(View.GONE);
                            myviewholder.donutProgress.setVisibility(View.GONE);
                            myviewholder.setmedia(pst.getPost1().getVideo());
                        }


                        myviewholder.contaoner.setId(position + 1);
                        myviewholder.setImage(pst.getPost1().getThumbnail());


                    }


                    myviewholder.setPost(pst.getPost1().getPost());
                    myviewholder.setDate(pst.getPost1().getPostedtime());

                    myviewholder.button.setChecked(pst.getPost1().isClap());
                    if (pst.getPost1().getClaps() != null) {
                        myviewholder.claps.setText(pst.getPost1().getClaps() + " claps");
                    }

                    myviewholder.button.setEventListener(new SparkEventListener() {
                        @Override
                        public void onEvent(ImageView button, final boolean buttonState) {
                            final Pst ps = getData().get(position);
                            if (!buttonState) {
                                String[] strings = myviewholder.claps.getText().toString().split(" ");

                                myviewholder.claps.setText(String.valueOf(Integer.parseInt(strings[0]) - 1) + " claps");
                                if (realm != null) {
                                    realm.executeTransaction(new Realm.Transaction() {
                                        @Override
                                        public void execute(Realm realm) {
                    ...
                    myviewholder.ply.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            v.setVisibility(View.INVISIBLE);
                            myviewholder.progressBar.setVisibility(View.VISIBLE);
                            myviewholder.donutProgress.setOnClickListener(new View.OnClickListener() {
                                @Override
                                public void onClick(View v) {
                            ...
                            final DonutProgress progress = myviewholder.donutProgress;
                                    ...
                                    if (realm != null) {
                                        realm.executeTransaction(new Realm.Transaction() {
                                            ...
                                            public void execute(Realm realm) {
                                                ...
                                                myviewholder.setmedia(selection);
...
// have a ProgressBar get updated automatically with the percent
                                            .progressBar(myviewholder.progressBar)
...
// can also use a custom callback
                                            .progressHandler(new ProgressCallback() {
                                                ...
                                                public void onProgress(long downloaded, long total) {
                                                    ...
                                                    if (donutProgress3 != null) {
                                                        donutProgress3.setVisibility(View.VISIBLE);
                                                        myviewholder.donutProgress.setProgress((int) ((100 * downloaded) / total));
                                                    } else {
                                                        myviewholder.donutProgress.setProgress((int) ((100 * downloaded) / total));
                                                    }
                            ...
                            downloadholders.add(new Downloadholders(p.getId(), file, file.getPath(), myviewholder.getAdapterPosition(), myviewholder, builder));
...
break;
                case 1:
                    ...
                    if (pst.isPosting()){
                        audioViewholder.play.setVisibility(View.GONE);
                        if (postin != null) {
                            if (!postin.contains(position)) {
                                postin.add(position);
                                final Pst pst1 = getData().get(position);
                                Uri uri=Uri.fromFile(new File(pst1.getPost1().getAudio()));
                                ...
                                post1.setPostedtime(pst1.getPost1().getPostedtime());
                                post1.setClap(pst1.getPost1().isClap());
                                post1.setType(pst1.getPost1().getType());
                                post.setId(pst1.getId());
                                post.setPost1(post1);

audioViewholder.progressBar.setVisibility(View.VISIBLE);
                                audioViewholder.donutProgress.setVisibility(View.VISIBLE);
                                audioViewholder.donutProgress.setOnClickListener(new View.OnClickListener() {
                                    @Override
                                    public void onClick(View v) {
                                        AlertDialog.Builder builder = new AlertDialog.Builder(context);
                                        builder.setMessage("Are you sure you want to remove this post?");
                                        ...
                                        builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                                            ...
                                            public void onClick(DialogInterface dialog, int which) {
                                                ...
                                                if (tsks != null && tsks.size() > 0) {
                                                     if (tsks.get(position).getTask() != null && tsks.get(position).getTask().isInProgress()) {
                                                        tsks.get(position).getTask().cancel();
                                                    ...
                                                    realm.executeTransaction(new Realm.Transaction() {
                                                        @Override
                                                        public void execute(Realm realm) {
                                                            Pst ps = realm.where(Pst.class).equalTo("id", tsks.get(position).getPost().getId()).findFirst();
                                                            ps.deleteFromRealm();
                                ...
                                final StorageReference storageReference = Config.storageReference.child(Config.auth.getCurrentUser().getDisplayName()).child("audio/" + uri.getLastPathSegment());
                                taskk = storageReference.putFile(uri);
                                tsks.put(position, new Uploadaholders(null, taskk, position, null,post));
                                taskk.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                                    @Override
                                    public Task<Uri> then(@NonNull Task<UploadTask.TaskSnapshot> task) throws Exception {
                                    ...
                                    public void onComplete(@NonNull Task<Uri> task) {
                                        if (task.isSuccessful()) {
                                            download = task.getResult().toString();
tsks.get(audioViewholder.getAdapterPosition()).getPost().getPost1().setName(jina);
tsks.get(audioViewholder.getAdapterPosition()).getPost().getPost1().setAudio(download);
                                            //  mProgressDialog.dismiss();
                                            Config.firestore.document(Config.auth.getCurrentUser().getDisplayName()).collection("posts").document(tsks.get(audioViewholder.getAdapterPosition()).getPost().getId())
                                                    .set(tsks.get(audioViewholder.getAdapterPosition()).getPost()).addOnCompleteListener(new OnCompleteListener<Void>() {
                                                @Override
                                                public void onComplete(@NonNull Task<Void> task) {
                                                    if (task.isSuccessful()) {
                                                        audioViewholder.donutProgress.setVisibility(View.GONE);
                                                        audioViewholder.progressBar.setVisibility(View.GONE);
                                                        audioViewholder.play.setVisibility(View.VISIBLE);
                                                        SharedPreferences.Editor editor = context.getSharedPreferences("id", MODE_PRIVATE).edit();
                                                        editor.putString("ids", tsks.get(audioViewholder.getAdapterPosition()).getPost().getId());
                                                        editor.apply();
                                                        realm.executeTransaction(new Realm.Transaction() {
                                                            @Override
                                                            public void execute(Realm realm) {
                                                                Pst ps = realm.where(Pst.class).equalTo("id", tsks.get(audioViewholder.getAdapterPosition()).getPost().getId()).findFirst();
                                                                assert ps != null;
                                                                ps.setPosting(false);
                                                                postin.remove(Integer.valueOf(position));
                                                                tsks.remove(audioViewholder.getAdapterPosition());
                                ...
                                taskk.addOnProgressListener(new OnProgressListener<UploadTask.TaskSnapshot>() {
                                    ...
                                    public void onProgress(UploadTask.TaskSnapshot taskSnapshot) {
                                        int p = (int) ((100.0 * taskSnapshot.getBytesTransferred()) / taskSnapshot.getTotalByteCount());
                                        audioViewholder.donutProgress.setProgress(p);
                    ...
                    audioViewholder.setdate(pst.getPost1().getPostedtime());
                    audioViewholder.setclap(pst.getPost1().getClaps());
                    audioViewholder.setplay(pst.getPost1().getAudio(),activity);
                    audioViewholder.button.setChecked(pst.getPost1().isClap());
                    audioViewholder.button.setEventListener(new SparkEventListener() {
                        @Override
                        public void onEvent(ImageView button, final boolean buttonState) {
                            final Pst ps = getData().get(position);
                            if (!buttonState) {
                                String[] strings = audioViewholder.clap.getText().toString().split(" ");

                                audioViewholder.clap.setText(String.valueOf(Integer.parseInt(strings[0]) - 1) + " claps");
                                if (realm != null) {
                                    realm.executeTransaction(new Realm.Transaction() {
                                        @Override
                                        public void execute(Realm realm) {
...
//holder.setIsRecyclable(false);
    ...
    public class Myviewholder extends RecyclerView.ViewHolder implements ToroPlayer {
        ...
        public boolean wantsToPlay() {
            ...
            return ToroUtil.visibleAreaOffset(this, itemView.getParent()) >= 0;
        }
